...
def set_selection_method(config):
    """
    Given the configuration settings, tGradientBoostingRegresshis function instantiates the configured
...
def set_optimization_params(opt):
    ...
    for key, item in opt.items():
        ...
        if isinstance(item, list) and assert_number(item):
...
def set_learning_method(config, X_train, y_train):
    ...
    if learning_cfg:
        ...
        if method_name == "SVR":
            ...
            if o:
                tune_params = set_optimization_params(o)
                estimator =optimize_model(RandomForestRegressor(), X_train, y_train,
                                          tune_params,
                                          scorers,
                                          o.get("cv", 5),
                                          o.get("verbose", True),
            ...
            if o:
                tune_params = set_optimization_params(o)
                estimator = optimize_model(Lasso(), X_train, y_train,
                                           tune_params,
                                           scorers,
                                           o.get("cv", 5),
                                           o.get("verbose", True),
...
def fit_predict(config, X_train, y_train, X_test=None, y_test=None, ref_thd=None):
    ...
    transformer = set_selection_method(config) #特征选择方法
    ...
    #################################################
    """
    log.info("feature_importances_")
    data = []
    for ff in enumerate(estimator.feature_importances_):
        data.append(ff)
    result=sorted(data,key=lambda x:x[1],reverse=True)
    with open('feature_importance.txt', 'w') as fp:
        for lines in result:
            fp.writelines(str(lines[0]+1)+" "+str(lines[1])+"\n")
    """
    #################################################
    file_dir='/home/moses/workspace/QE_project/learning'
    #################################################
	...
	log.info("Evaluating prediction on the test set...")
        ...
        with open(file_dir+"/predicted.csv", 'w') as _fout:
            for i, _y in enumerate(zip(y_test, y_hat)):
                print >> _fout,  "%s\t%d\t%f\t%d" % ('JXNU/Emb+RNNLM+QuEst+SVM',i+1,_y[1],get_index(i,y_hat))
    ...
    if (X_test is not None) and (y_test is None):
        ...
        with open(file_dir+"/predicted.csv", 'w') as _fout:
            for i, _y in enumerate(y_hat):
                print >> _fout,  "%s\t%d\t%f\t%d" % ('JXNU/Emb+RNNLM+QuEst+SVM',i+1,_y,get_index(i,y_hat))
...
def main(argv=None): # IGNORE:C0111
    ...
    program_name = os.path.basename(sys.argv[0]) #获取程序名：learn_model.py
...
''' % (program_shortdesc, str(__date__))
    ...
    try:
        ...
        cfg_path = args.configuration_file #获取配置文件
