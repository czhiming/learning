...
def set_optimization_params(opt):
    ...
    for key, item in opt.items():
        ...
        if isinstance(item, list) and (len(item) == 3) and assert_number(item):
            ...
            #print key, item
            params[key] = item
        elif isinstance(item,list) and assert_number(item):
            params[key] = item
...
def set_learning_method(config, X_train, y_train):
    ...
    if learning_cfg:
		...
		estimator = RandomForestRegressor(n_estimators=p.get("n_estimators",100),
...
def get_order_index(y_hat):
    y_hat_ = sorted(enumerate(y_hat),key=lambda x:x[1],reverse=True)
    index = []
    for key,value in enumerate(y_hat_):
        index.append(value[0]+1)
    return index
